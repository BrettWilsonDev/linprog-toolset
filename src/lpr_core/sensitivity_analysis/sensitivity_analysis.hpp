...
// Revised solve_linear to extract coefficients more reliably
Symbolic solve_linear(const Symbolic &expr, const Symbol &d)
{
...
// Revised to_double for robust conversion
double to_double(const Symbolic &sym)
{
    try
    {
    ...
    {
        std::cerr << "Error in to_double: " << e.what() << std::endl;
        throw;
    }
}
...
public:
    ...
    void doSensitivityAnalysis(const vector<Symbolic> &objfunc, const vector<vector<Symbolic>> &constraints, bool isMin, bool absRule, bool optTabLockState)
    {
        vector<Symbolic> a = objfunc;
        vector<vector<Symbolic>> b = constraints;

        auto [ct, mCbv, mB, mBNegOne, mCbvNegOne, bvs] = mathPrelim.DoPreliminaries(a, b, isMin, absRule, optTabLockState);
        changingTable = ct;
        ...
        if (objFuncHasDelta)
        {
            for (size_t i = 0; i < changingTable[0].size() - 1; ++i)
            {
                ...
                {
                    Symbolic delta = solve_linear(e, d);
        ...
        if (constraintsHasDelta)
        {
            if (conStraintDeltaCol != -1 && changingTable[0][conStraintDeltaCol] != changingTable[0][conStraintDeltaCol].subst(d == Symbolic(0)))
            {
                try
                {
                    Symbolic delta = solve_linear(changingTable[0][conStraintDeltaCol], d);
                    deltasList.push_back(delta);
                }
                catch (...)
                {
        ...
        if (!deltasList.empty())
        {
            ...
            {
                all_deltas.push_back(to_double(del));
            }
        ...
        double twd = to_double(termWithoutdelta);
    ...
    void test()
    {
        ...
        try
        {
            doSensitivityAnalysis(customObjFunc, customConstraints, false, false, false);
        }
        catch (const std::exception &e)
        {
